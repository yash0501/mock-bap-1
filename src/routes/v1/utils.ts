...
export const getAllUseCases = async () => {
  ...
  for (let index = 0; index < domains.length; index++) {
    ...
    const dir = await fs.readdir("./../mock_json_files/" + domain, {
      withFileTypes: true,
    });
...
export const isValidUseCase = async (
  domain: string,
  use_case: string
): Promise<boolean> => {
...
export const getMockResponse = async (
  ...
  const req_data = await fs.readFile(
...
export const initiateSearch = async (req: Request) => {
  try {
    ...
    const res = await axios.post(end_point, search_request, {
      headers: { use_case },
    });
    // console.log(res);
...
export const validateTriggerRequest = async (req: Request, api: string) => {
  ...
  if (api !== "search") {
    if (bpp_uri === undefined || bpp_uri === "") {
      console.log(
        `${config.get("bap_id")} ERROR : Invalid inputs bpp_uri required`
      );
      return { message: `bpp_uri is a required input` };
    }
    if (transaction_id === undefined || transaction_id === "") {
      console.log(
        `${config.get("bap_id")} ERROR : Invalid inputs transaction_id required`
      );
      return { message: `transaction_id is a required input` };
    }
  }
  const valid_use_case = await isValidUseCase(
    req.body.domain,
    req.body.use_case
  );
  if (!valid_use_case) {
